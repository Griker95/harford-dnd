...
- Para jugadores en turnos, refrescar `entry.nameColor` desde TRP3 cuando se renderiza la carta si el perfil esta disponible. Al anadir un jugador, guardar `entry.trpUnitID` y `entry.trpProfileID` cuando TRP3 los exponga. Al abrir ficha de jugador, seguir el patron del directorio TRP3: primero `TRP3_API.register.getProfile(profileID)` si hay `entry.trpProfileID`; despues resolver por `entry.trpUnitID` con `getUnitIDProfileID`/`hasProfile` + `getProfile`; despues probar variantes (`entry.unitName`, `entry.name`, con/sin reino); y solo como fallback buscar unidades vivas por GUID/nombre (`player`, `target`, `mouseover`, `focus`, party/raid). Los jugadores no se cargan como companions/NPC.
