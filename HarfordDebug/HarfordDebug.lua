...
-- ¿Dispara PLAYER_STARTED_MOVING en Epsilon? AGENTS.md dice que puede no hacerlo, y de eso
-- depende que la ventana de loot te quite la "Marca de loot" (aura 224063, visible para todos)
-- al alejarte del cadaver. Sin dato no hay decision: esto lo mide en vez de suponerlo.
do
    local sonda, cuenta = nil, { arranca = 0, para = 0, flag = 0 }
    API.RegisterCommand("movevento", function(args)
        local cmd = tostring(args or ""):match("^%s*(%S*)")

        if cmd == "parar" then
            if sonda then sonda:UnregisterAllEvents(); sonda:SetScript("OnUpdate", nil) end
            Print(string.format("Sonda parada. Eventos: STARTED=%d STOPPED=%d.  IsPlayerMoving cambio %d vez/veces.",
                cuenta.arranca, cuenta.para, cuenta.flag))
            Print(cuenta.arranca == 0 and cuenta.flag > 0
                and "|cffff5555El evento NO dispara, pero moverte SI se detecta con GetUnitSpeed.|r"
                or (cuenta.arranca > 0 and "|cff55ff55El evento dispara: la ventana de loot se cierra bien al moverte.|r"
                    or "|cffffcc00No te has movido durante la prueba.|r"))
            return
        end

        cuenta.arranca, cuenta.para, cuenta.flag = 0, 0, 0
        sonda = sonda or CreateFrame("Frame")
        sonda:SetScript("OnEvent", function(_, event)
            if event == "PLAYER_STARTED_MOVING" then
                cuenta.arranca = cuenta.arranca + 1
                Print("|cff55ff55PLAYER_STARTED_MOVING|r (" .. cuenta.arranca .. ")")
            else
                cuenta.para = cuenta.para + 1
            end
        end)
        sonda:RegisterEvent("PLAYER_STARTED_MOVING")
        sonda:RegisterEvent("PLAYER_STOPPED_MOVING")
        -- Contraste: si la velocidad cambia y el evento no llega, el evento es el que falla.
        -- OnUpdate acotado a la prueba, que se retira con `parar`.
        local moviendose = false
        sonda:SetScript("OnUpdate", function()
            local ahora = (GetUnitSpeed and GetUnitSpeed("player") or 0) > 0
            if ahora ~= moviendose then
                moviendose = ahora
                if ahora then cuenta.flag = cuenta.flag + 1 end
            end
        end)
        Print("Sonda activa. Camina un poco y luego |cffffd100movevento parar|r.")
    end, "¿dispara PLAYER_STARTED_MOVING en Epsilon? (afecta al cierre de la ventana de loot)")
end
