...
local function ShowCastFrame(spell)
    if not CastFrame then return end
    local cost = API.GetSpellCost and API.GetSpellCost(spell) or API.GetManaCost(spell.level)
    local manaCur = API.GetManaCurrent and API.GetManaCurrent() or 0
    local manaMax = API.GetManaMax and API.GetManaMax() or 0
    local canCast, reason = API.CanCast and API.CanCast(spell.id)
    ...
    CastFrame.summary:SetText(
        "Coste si tiene exito: " .. tostring(cost) .. " mana\n" ..
...
local function PrintMessage(text)
    if DEFAULT_CHAT_FRAME then
        DEFAULT_CHAT_FRAME:AddMessage("|cff35a8ffHarfordCompendio:|r " .. text)
    end
end
...
local function CreateCastFrame()
    ...
    CastFrame.success:SetScript("OnClick", function()
        if CastFrame.spell then
            ...
            local ok, costOrErr = resolver(CastFrame.spell.id)
            if not ok then
    ...
    CastFrame.ritual:SetScript("OnClick", function()
        if CastFrame.spell and (API.ResolveCast or API.ConfirmCast) then
            local resolver = API.ResolveCast or API.ConfirmCast
            local ok, err = resolver(CastFrame.spell.id, { ritual = true })
            if not ok then
...
local function IsDMModeActive()
    if C_Epsilon then
        if type(C_Epsilon.IsDM) == "function" then
            local ok, value = pcall(C_Epsilon.IsDM)
            if ok then return value == true end
        elseif C_Epsilon.IsDM ~= nil then
            return C_Epsilon.IsDM == true
        end
    end
...
local function GetPhaseRankLabel()
    if C_Epsilon then
        if C_Epsilon.IsOwner and C_Epsilon.IsOwner() then return "Owner" end
        if C_Epsilon.IsOfficer and C_Epsilon.IsOfficer() then return "Oficial" end
        if C_Epsilon.IsMember and C_Epsilon.IsMember() then return "Miembro" end
    end
    if ARC and ARC.PHASE then
        if ARC.PHASE.IsOwner and ARC.PHASE.IsOwner() then return "Owner" end
        if ARC.PHASE.IsOfficer and ARC.PHASE.IsOfficer() then return "Oficial" end
        if ARC.PHASE.IsMember and ARC.PHASE.IsMember() then return "Miembro" end
    end
    return "No detectado"
end
...
local function RefreshMasterFrame()
    ...
    MasterFrame.statusText:SetText(table.concat({
        ...
        "",
        "Este panel solo se abre con .ph dm on activo.",
...
local function ShowMasterFrame()
    if not IsDMModeActive() then
        PrintMessage("Panel del Master disponible solo con .ph dm on activo.")
        return
    end
...
init:SetScript("OnEvent", function()
    API.Init()
    PrintMessage("cargado. Usa /harford compendio.")
end)
