...
commFrame:SetScript("OnEvent", function(_, event, ...)
    ...
    elseif event == "CHAT_MSG_ADDON" then
		...
		if prefix == COMM_PREFIX then
			...
			elseif type(message) == "string" and message:find("^LOOTC|") then
				guid, lootTable = HarfordSync.ReceiveTaggedLootChunk(message, sender)
				if guid and lootTable then
					HarfordLootAPI.SetResolvedLoot(guid, lootTable, false)
				end
			end
...
local function AdvanceLootPageIfCurrentButtonsHidden()
    ...
    if allButtonsHidden and HarfordLootFrameDownButton and HarfordLootFrameDownButton:IsShown() then
...
local function LootItem(slot, entry, quantity)
    ...
    if ((slot > 0) and (slot < (numLootToShow + 1))) then
        ...
        if (button) then
            if button._harfordLootPending then
                return
            end
            ...
            local itemId = HarfordLootFrame.lootTable[entry][1]
            button._harfordLootPending = true
            -- Salvaguarda: si el servidor NUNCA confirma (callback perdido), el slot quedaria
            -- bloqueado para siempre. Un timeout libera el flag para poder reintentar. El token evita
            -- que este timeout libere un intento POSTERIOR sobre el mismo boton.
            button._harfordLootToken = (button._harfordLootToken or 0) + 1
            local pendingToken = button._harfordLootToken
            if C_Timer and C_Timer.After then
                C_Timer.After(12, function()
                    if button._harfordLootPending and button._harfordLootToken == pendingToken then
            ...
            local ok, err = HarfordServerActions.GiveItem(itemId, quantity, {
                ...
                callback = function(success, messages)
                    ...
                    if not HarfordLootFrame.lootTable or not HarfordLootFrame.lootTable[entry] then
                        return
                    end
                    ...
                    button:Hide()
                    AdvanceLootPageIfCurrentButtonsHidden()
                end,
            })
            if not ok then
                button._harfordLootPending = nil
