...
- Marco visual ToT (`artFrame`): `totBarsOverlay.artFrame` es también hijo de `UIParent`/MEDIUM (level 82), por debajo de las barras overlay propias pero por encima de las barras extra del unitframe. Clona en runtime solo la textura real `UI-TargetofTargetFrame` del hijo `TargetFrameToTTextureFrame` / `FocusFrameToTTextureFrame` usando bounds relativos. No clonar regiones del root, atlas ambiguos ni hijos debuff/buff/portrait: el debug real mostro `TargetFrameToTDebuff1..4` ocultos con regiones `Texture`, y copiar esas regiones al `artFrame` genera los cuatro cuadrados falsos. `UpdateToTArtOverlay(root, ov)` es genérico y acepta cualquier frame ToT como primer parámetro. Si el ToT se oculta o no existe `targettarget`, `artFrame` se oculta junto al resto.
- ToT portrait overlay (`totBarsOverlay.portraitFrame`): frame `UIParent`/MEDIUM level 85, `SetAllPoints(TargetFrameToTPortrait)`. En Epsilon `TargetFrameToTPortrait`/`FocusFrameToTPortrait` son `Texture`, no `Frame`; no se puede usar `HookScript("OnShow")` sobre ellos. Textura `ptex` con `SetTexCoord(0.08,0.92,0.08,0.92)` + mascara circular via `CreateMaskTexture`/`AddMaskTexture` con `TEX_PORTRAIT_MASK`. **CRÍTICO: la máscara debe aplicarse a AMBAS texturas — `pbg:AddMaskTexture(mask)` Y `ptex:AddMaskTexture(mask)` — usando un único mask object con `mask:SetAllPoints(pf)` (frame completo). Si solo se aplica a `ptex`, el `pbg` queda cuadrado y se ve como borde oscuro alrededor del icono circular.** El portrait nativo se sincroniza por alpha despues de cada update: alpha `0` si nuestro overlay de icono esta visible, alpha `1` si no hay overlay. No intentar ocultarlo permanentemente con hooks de frame.
...
- Enfoques que FALLARON y no deben reintentarse:
  ...
  - **`totBarsOverlay` como hijos de `TargetFrameToT`**: frame level +10 sobre barras nativas. Epsilon no honra strata cross-tree: `barSlotOverlays` (MEDIUM strata) renderizaban encima del ToT (HIGH strata) aunque el level absoluto fuese mayor. Solucion correcta: UIParent/HIGH strata.
  ...
  - **Hijos de `TargetFrameToT` para portrait overlay**: el portrait nativo renderiza en Epsilon encima de overlays del mismo arbol independientemente del frame level. Solucion: overlay UIParent/DIALOG + sincronizar alpha de la `Texture` nativa tras `TargetofTarget_Update`/`FocusofTarget_Update`.
  - **Clonar todas las regiones del ToT en `artFrame`**: fallido. Copia regiones de debuffs ocultos y/o root regions ambiguas (`TargetPortrait1`), creando cuadrados falsos y errores visuales de icono. Solo copiar `*ToTTextureFrame`.
...
- En overlays de party/raid no copiar `nativeBar:GetStatusBarTexture()` al `StatusBar` Harford. En Epsilon/SL puede devolver atlas/texturas ya coloreadas o con material no neutro; al multiplicarlo con `SetStatusBarColor` produce barras negras/rojas. Usar textura estable `HarfordUnitFrames.C.TEX_STATUS` y aplicar color Harford/clase/recurso encima.
...
- En frames de grupo/party, el hover de recursos no debe mostrar el nombre del recurso: solo el valor corto (`cur/max`, `%` o ambos segun config). Si la opcion de texto ya muestra numero/porcentaje/ambos, el hover de party no debe cambiar visualmente nada. En raid se mantiene el hover completo con nombre de recurso.
