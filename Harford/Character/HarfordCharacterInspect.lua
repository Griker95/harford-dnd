...
local function DropSnapshotAliases(snap)
    ...
    if snap.profileName and snap.profileName ~= "" then
        aliases[#aliases + 1] = snap.profileName
    end
    for _, alias in ipairs(aliases) do
        if HarfordDnDProgression and HarfordDnDProgression.ClearInspectData then
            HarfordDnDProgression.ClearInspectData(alias)
        end
        if HarfordDnDItems and HarfordDnDItems.ClearInspectData then
            HarfordDnDItems.ClearInspectData(alias)
        end
    end
end
...
local function PruneInspectCache()
    ...
    for _, name in ipairs(inspectCacheOrder) do
        ...
        if type(snap) == "table" and not unique[snap] then
            -- `updatedAt`/`requestedAt` valen 0 en un snapshot recien pedido; `tonumber(0)` es 0 (no
            -- nil), asi que `A or B` NO cae al fallback. Elegir la referencia explicitamente para no
            -- podar un snapshot recien solicitado (que aun tiene updatedAt=0) por "demasiado viejo".
            local ref = tonumber(snap.updatedAt) or 0
            if ref <= 0 then ref = tonumber(snap.requestedAt) or 0 end
            local age = ref > 0 and (now - ref) or 0
...
local function IsExpectedResponse(sender, profileName)
    local now = GetTime and GetTime() or 0
    local candidates = {}
    local function add(name)
        ...
        local short = ShortName(name)
        if short and short ~= "" then candidates[#candidates + 1] = short end
    end
    ...
    for _, name in ipairs(candidates) do
        local snap = API.Cache[name]
        if type(snap) == "table" and (tonumber(snap.requestedAt) or 0) > 0 then
            if (now - (tonumber(snap.requestedAt) or now)) <= INSPECT_CACHE_TTL then
                return true
            end
        end
    end
...
function API.NoteResources(sender, profileName, tbl)
    if not IsExpectedResponse(sender, profileName) then return end
...
function API.NoteProgression(profileName, data)
    -- Igual que NoteResources: solo aceptar si hay una peticion de inspeccion reciente para este
    -- perfil. Sin esto, un DNDINSCLASS no solicitado escribia en la cache efimera (rompe el contrato
    -- "snapshots no solicitados se ignoran").
    if not IsExpectedResponse(nil, profileName) then return end
...
function API.SendSnapshotTo(targetName)
    ...
    local ok, err = HarfordSync.Send(ADDON_PREFIX, SerializeBase(profileName, base), "WHISPER", targetName)
    if not ok then return false, err end

    if HarfordDnDNet and HarfordDnDNet.SendResourceResponseTo then
        ok, err = HarfordDnDNet.SendResourceResponseTo(targetName)
    ...
    if HarfordDnDProgression and HarfordDnDProgression.Export and HarfordSync.SendDnDClassProgression then
        ok, err = HarfordSync.SendDnDClassProgression(ADDON_PREFIX, profileName, HarfordDnDProgression.Export(profileName), "WHISPER", targetName, "DNDINSCLASS")
        if not ok then return false, err end
    end
    if HarfordDnDItems and HarfordDnDItems.GetEquipment and HarfordSync.SendDnDEquipment then
        ok, err = HarfordSync.SendDnDEquipment(ADDON_PREFIX, profileName, HarfordDnDItems.GetEquipment(profileName), "WHISPER", targetName, "DNDINSEQUIP")
        if not ok then return false, err end
    end
    return true
end
...
function API.HandleAddonMessage(prefix, message, sender)
    ...
    if profileName and base then
        if not IsExpectedResponse(sender, profileName) then return true end
        local snap = EnsureSnapshot(profileName)
        snap.sender = sender or snap.sender
        snap.base = base
        MarkUpdated(snap)
    ...
    if op == "DNDINSCLASS" then
        ...
        if pn and data and IsExpectedResponse(sender, pn) then API.NoteProgression(pn, data) end
        return true
    ...
    elseif op == "DNDINSEQUIP" then
        ...
        if pn and data and IsExpectedResponse(sender, pn) then API.NoteEquipment(pn, data) end
        return true
