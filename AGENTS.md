...
Harford/
  ...
  HarfordCharacterPanel.lua   -- panel de personaje: Ficha, Creacion, Subida y acceso a Reputacion
...
- Contrato `HarfordDnDRolls` (`Harford/HarfordDnDRolls.lua`): serializacion, recepcion visual y emision de tiradas. API publica: `GetDisplayName`, `Serialize`, `Deserialize`, `DisplayInChat`, `Broadcast`. Mantiene el formato historico de 10 campos separados por `^` en prefix `DND5EARC`, incluyendo `nameColor` en el campo 10. Los campos de texto se escapan de forma compatible (`%`, `^`, saltos de linea) para que nombres compuestos, links TRP3 o etiquetas raras no rompan el parser; `Deserialize` sigue aceptando mensajes antiguos sin escapes. `Broadcast` usa `HarfordSync.BestChannel()`/`HarfordSync.Send(...)`, loguea por `HarfordDebug` si el envio falla o el payload se acerca al limite seguro, pinta la tirada local y reproduce el sound kit TRP3 `36629` solo en emision local real; `DisplayInChat` no reproduce sonido porque tambien se usa para tiradas recibidas.
- Contrato `HarfordDnDData` (`Harford/HarfordDnDData.lua`): solo datos. `HarfordDnDData.ABIL` (6 caracteristicas, `{key, short, desc, saveDesc}`) y `HarfordDnDData.SKILLS` (18 habilidades, `{name, ability, id, desc}`). Se referencian como `HarfordDnDData.ABIL`/`.SKILLS` (no aliases locales). **`desc`/`saveDesc`/`desc` son la fuente UNICA de descripciones de caracteristica/salvacion/habilidad** para los tooltips del panel (`HarfordCharacterPanel` deriva `ABILITY_TOOLTIP_TEXT` de `ABIL[].desc`; las vistas Habilidades/Salvaciones leen `skill.desc`/`abil.saveDesc`). No duplicar estos textos en otros modulos.
- Contrato `HarfordDnDBook` (`Harford/HarfordDnDBook.lua`): libro hardcodeado de clases/subclases/rasgos. Solo datos y helpers puros (`GetClasses`, `GetClass`, `GetSubclass`, `GetUnlockedFeatures`, `FindClassIdByText`). Los rasgos exponen `effects` declarativos; nunca contienen Lua ejecutable. **Sistema: World of Warcraft D&D 5ª Edicion (homebrew español)** — fuente: `Warcraft 5º Edición_compressed.pdf` / GMBinder `Warcraft_5DnD_GMBinder.txt`. Las 12 clases son las de WoW: Caballero de la Muerte, Cazador de Demonios, Druida, Cazador, Mago, Monje, Paladin, Sacerdote, Picaro, Chaman, Brujo, Guerrero (cada una con 3 especializaciones como subclases). **No quedan vestigios de las clases D&D de relleno.** Contenido de rasgos **hasta nivel 6 COMPLETO en las 12 clases** (con sus 36 subclases y rasgos de subclase a sus niveles). Cada clase tiene sus salvaciones tomadas del manual ESPAÑOL ("Tiradas de Salvacion"). Automatizado donde aplica: estilos de combate (`armorClass`/`weaponAttack`/`weaponDamage`), ASI L4, pericias de clase/subclase (`skillExpertise`), y salvaciones por choice (`saveProf`: Druida Feral, Chaman Tierra). El resto (conjuros, recursos de clase, mecanicas complejas) va como `informativo`/`recurso` con su texto del manual. Pendiente futuro: niveles 7+ y automatizaciones mas finas. Helper local `ASI(classId, level)` para el rasgo de Mejora de Caracteristica (choice `ability+1` x2). **Rasgos de subclase**: cada `subclass` puede tener su propia lista `features`; `GetUnlockedFeatures` incluye las de la subclase SELECCIONADA (`entry.subclassId`) cuyo nivel <= nivel, con `className = "Clase / Subclase"`. Las salvaciones se toman del manual ESPAÑOL ("Tiradas de Salvacion" de cada clase), que puede diferir de tablas inglesas (p.ej. Caballero de la Muerte = Constitucion/Carisma). Clases ya rellenas a L6: `guerrero`, `caballero_muerte` (3 presencias a L3), `cazador_demonios` (3 marcas a L3 y L6; "Competencia Adicional" da `skillExpertise`), `druida` (3 sendas a L2 y L6; subclase elegida a nivel 2; Feral "Adaptacion Salvaje" es choice de `saveProf` Destreza/Constitucion), `cazador` (3 arquetipos a L3 y L5; Supervivencia "Estudiante de lo Salvaje" da `skillExpertise` Supervivencia; estilo Tiro con Arco/Tirador suma `weaponAttack`), `mago` (3 estudios a L2 y L6; subclase a nivel 2; Arcano "Brillantez Arcana" da `skillExpertise` Arcano), `monje` (3 tradiciones a L3 y L6; rasgo de clase "Golpes Empoderados por el Chi" a L6), `paladin` (3 caminos a L3; rasgo de clase "Aura de Proteccion" a L6; estilo Defensa/Doble Empuñadura automatizados), `sacerdote` (3 llamados elegidos a nivel 1, rasgos a L1 y L6; Sagrado "Competencia Adicional" da `skillExpertise` Religion), `picaro` (Pericia = choice de `skillExpertise` a L1 y L6; 3 arquetipos a L3), `chaman` (Afinidad Elemental choice con Tierra→`saveProf` Constitucion; 3 vinculos a L3 y L6), `brujo` (saves Constitucion/Inteligencia; 3 estudios elegidos a L2 con rasgos a L2 y L6). **Las 12 clases completas a L6.** Lo no automatizable va como `informativo` con su texto del manual (ojo: hay errores de traduccion en hechizos/descripciones). Futuros añadidos de libros D&D iran como clases nuevas, sin mezclarse con el sistema Warcraft. **Descripciones**: cada clase y cada subclase tiene un campo `desc` (1-2 frases, condensado del manual) que `HarfordCharacterPanel` muestra como tooltip nativo (`tooltipTitle`/`tooltipText`/`tooltipOnButton`) en los dropdowns de seleccion de Clase/Subclase de `Subida`. Es la fuente unica de la descripcion de clase; no reescribir esos textos en la UI.
...
- Contrato `HarfordDnDFeatureEffects` (`Harford/HarfordDnDFeatureEffects.lua`): interpreta los efectos activos de progresion y devuelve una capa derivada para calculos. Efectos soportados: `bonus` a `ability/save/skill/initiative/weaponAttack/weaponDamage/armorClass/spellAttack/spellDC`, `saveProf`, `skillProf`, `skillExpertise`, `resourceMax`, `armorProf` (`effect.armor`), `weaponProf` (`effect.weapon`). **Competencias (4 categorias)**: salvacion (`saveProf`), habilidad (`skillProf`/`skillExpertise`), **armadura** (`armorProf`; claves `ligera`/`media`/`pesada`/`escudo`) y **arma** (`weaponProf`; claves `sencillas`/`marciales`/`armas de fuego` o arma concreta). Base de la CLASE via `classDef.armorProfs`/`classDef.weaponProfs` (union de todas las clases en multiclase; las salvaciones solo de la primera). Raza/trasfondo/dote añaden via efectos `armorProf`/`weaponProf`. Consulta: `HasArmorProf`, `HasWeaponProf`, `GetArmorProfs`, `GetWeaponProfs`, `GetProficiencies` (instantanea `{armor, weapon, saves, skills}`). **Las 12 clases ya tienen `armorProfs`/`weaponProfs` pobladas desde el manual** (claves de armadura `ligera/media/pesada/escudo`; armas `sencillas/marciales/armas de fuego` + armas concretas tipo `gujas`, `espadas cortas`, `ballestas de mano`, etc.). **Pendiente: convertir los rasgos "Entrenamiento con armas" de razas de `informativo` a efectos `weaponProf`, y pintar la seccion de Competencias en el panel.** **Modificadores de caracteristica (ability) = de CREACION, no live**: los efectos `bonus ability` (incrementos de raza/trasfondo/dote) se enrutan a un bucket aparte `creationBonus.ability` y **NO se suman a la puntuacion en vivo** (`GetBonus("ability")` devuelve solo el bucket live `bonus.ability`, reservado para bonos/penalizaciones de **estado/objeto**, vacio por ahora). Motivo: al cargar una ficha (TRP3) los incrementos de raza ya vienen horneados en la puntuacion nativa; volver a sumarlos los duplicaria y los mostraria erroneamente en el tooltip de caracteristica. `GetCreationAbilityBonus(key)` expone el bucket de creacion para el **futuro pipeline de creacion de ficha dentro del addon** (ahi SI se aplicaran sobre la puntuacion base). En el tooltip de caracteristica del panel (`AbilityTooltipTitle`) el `(base+bonus)` solo refleja el bono live (estado/objeto), coloreado verde/rojo; los modificadores de raza no aparecen. Esta capa **suma sobre valores manuales** y no escribe permanentemente en `ARCGET/ARCSET`; los recursos maximos derivados solo afectan al calculo visual/runtime. **Salvaciones base de clase**: `Resolve` aplica ademas las `classDef.saves` de la **primera** clase de `classLevels` como `saveProf` (regla 5e: en multiclase solo la primera clase otorga sus salvaciones). Se suma al flag manual de salvacion en `HarfordDnDCalc` (no marca el checkbox; suma PB al calculo). **Choices**: por cada rasgo activo con `choice`, `Resolve` aplica los efectos de la(s) opcion(es) elegida(s) (`HarfordDnDProgression.GetChoice(featureId)` → optionId por slot; `HarfordDnDBook.GetChoiceOption`). La progresion guarda `choices[featureId] = { optSlot1, ... }` (`SetChoiceSlot`/`GetChoice`), viaja en el sync `DNDCLASS` (seccion `h=`, slots por `~`) y se elige en `HarfordCharacterPanel` > `Subida` con un dropdown por slot bajo cada rasgo.
- Sync de progresion: `HarfordSync` usa opcode `DNDCLASS` dentro del prefix `DND5EARC`. Secciones separadas por `;`: `v=`(schema), `c=`(clases por `~`), `f=`(featureStates), `h=`(choices, slots por `~`), `r=`(raza `id~subraceId`), `b=`(trasfondo `id`), `d=`(dotes, ids por `~`). Texto escapado con `EscapeProgressionText`. Va separado de `DNDCFG` y `DNDPROF`, soporta chunks `DNDCLASSC` si supera el limite seguro, y se envia junto a la ficha en `BroadcastConfig`, `BroadcastConfigForPlayer` y `BroadcastAll`. Receptores sin esta logica ignoran el opcode; receptores nuevos lo importan con `HarfordDnDProgression.Import`. Mensajes antiguos sin `r=`/`b=`/`d=` se deserializan con raza/trasfondo/dotes vacios.
...
- Contrato `HarfordCharacterPanel` (`Harford/HarfordCharacterPanel.lua`): panel de personaje para usuario final, abierto desde el icono de tabardo de la ficha o `/harfordchar`/`/hchar`. Primera pestaña: `Ficha` (resumen del PJ, caracteristicas y rasgos). `Creacion` permite preparar caracteristicas base (manual, array, compra por puntos como ayuda visual y 4d6). `Subida` edita progresion (`HarfordDnDProgression`: raza/subraza, trasfondo, dotes, mana, clase/subclase/nivel, rasgos y choices) usando los libros `HarfordDnDBook/Races/Backgrounds/Feats`. **No mostrar checkboxes por rasgo**: los booleanos de rasgos son internos; solo se muestran dropdowns/controles cuando existe una eleccion real (`choice`). `Reputacion` NO duplica ni reimplementa la lista: usa el propio `HarfordReputationUI` en modo embebido (`EmbedInto`/`DetachEmbedded`) dentro del panel de personaje. Composicion visual objetivo: estilo CharacterFrame/Shadowlands, retrato circular del jugador/target DM en cabecera, titulo dinamico y tabs inferiores (`Ficha`, `Reputacion`, `Creacion`, `Subida`). La ficha compacta conserva solo tabs de tirada (`Caracteristicas`, `Ataque`, `Habilidades`); la antigua pestaña `Clases` y su bloque `RefreshClassPanel` fueron retirados de `HarfordDnD.lua`.
  - Fuentes confirmadas para replicar el `CharacterFrame` nativo sin ajustes a ojo:
    ...
    - `G:\Epsilon\_retail_\WTF\Account\MORTYN\SavedVariables\Harford.lua`, tabla `HarfordFrameProbe`: captura generada por `/harforddebug probeframe CharacterFrame`; conserva atlas/texturas (`_UI-Frame-TitleTileBg`, `UI-Frame-InnerTopLeft`, `UI-Character-Info-Title`, etc.), puntos, tamaños, capas, fuentes y colores.
  - Enfoque correcto: usar esas capturas como referencia de geometria/arte para una reconstruccion controlada en `HarfordCharacterPanel`. No clonar recursivamente el `CharacterFrame` vivo con scripts/eventos/secure templates, porque arrastra pestañas nativas, `ReputationFrame`, `TokenFrame`, popups, eventos y riesgo de taint. Si se necesita actualizar la referencia, ejecutar `/harforddebug probeframe CharacterFrame`, hacer `/reload`, y leer de nuevo `SavedVariables\Harford.lua`.
...
- `HarfordReputationUI.Toggle()` → abre/cierra el panel. Llamado desde `/harfordrep` y desde la pestaña `Reputacion` de `HarfordCharacterPanel`.
...
- Los buffs/debuffs del `TargetFrame` y `FocusFrame` solo deben desplazarse si esa unidad tiene mas de dos barras visibles. El desplazamiento es dinamico segun la altura real de las barras extra (`frame.extraResourceHeight`), no con una formula paralela. No cambiar el parent de los frames de aura: guardar sus puntos originales con `SaveAuraPoints`, restaurar `point`, `relativeTo`, `relativePoint` y `x`, y modificar solo `y` con `-extraResourceHeight`. No reanclar al `TOPLEFT`/`BOTTOMLEFT` del frame Harford, porque eso pierde la posicion nativa y desplaza los iconos a izquierda/arriba. El cache de anclas esta separado por unidad (`target`/`focus`) para que un focus jugador con recursos extra no contamine la posicion de buffs del target ni al reves. Invalidar el cache de anclas al cambiar `UnitGUID(unit)` o en `UNIT_AURA target/focus`, porque Blizzard recoloca `*FrameBuff1`/`*FrameDebuff1`. Importante: si el primer debuff esta anclado al primer buff en el layout nativo, no aplicar el offset tambien al debuff; debe heredar el movimiento desde el buff para no solapar/desdoblar filas. Despues del desplazamiento, medir y normalizar la separacion entre primera fila de buffs y debuffs para conservar el gap nativo. Nombres: target intenta `_G.TargetFrameBuff1` / `_G.TargetFrameDebuff1`; focus intenta `_G.FocusFrameBuff1` / `_G.FocusFrameDebuff1`; fallback a `.BuffFrame` / `.DebuffFrame` del frame nativo.
- **Bug de drift de buff frames (confirmado y corregido)**: limpiar el cache de anclas sin restaurar primero los buff frames a su posicion nativa hace que la siguiente llamada a `SaveAuraPoints` guarde la posicion ya desplazada como base, y `ShiftAuraFrame` desplace de nuevo sobre ella. Cada evento `UNIT_AURA` acumulaba un offset extra hasta sacar los buff frames de pantalla ("congelados"). **Solucion**: restaurar siempre las posiciones nativas ANTES de limpiar el cache (`RestoreTargetAuras()` / `RestoreFocusAuras()` / `RestoreUnitAuras(unit)`). Nunca limpiar el cache sin restaurar primero.
