...
function HarfordDnDMinimap.Create()
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("/harford ficha minimap show", 0.7, 0.9, 0.7)
        GameTooltip:AddLine("/harford ficha minimap hide", 0.7, 0.9, 0.7)
        GameTooltip:Show()
    end)
    ...
    btn:SetScript("OnClick", function(_, button)
        if button == "LeftButton" then
            -- Nuevo punto de entrada: despliega "Herramientas de Rol". Fallback a la ficha.
            if HarfordToolTray and HarfordToolTray.Toggle then
                HarfordToolTray.Toggle()
            elseif _G.DND5E_ARC_API and _G.DND5E_ARC_API.Toggle then
                _G.DND5E_ARC_API.Toggle()
            end
        elseif button == "RightButton" then
            if _onReset then _onReset() end
            HarfordChat.Print("Posiciones de los marcos reiniciadas.")
        end
    end)
    ...
    -- Ancla la bandeja "Herramientas de Rol" a este boton (parent + flecha), como el Epsilon tray.
    if HarfordToolTray and HarfordToolTray.Attach then
        HarfordToolTray.Attach(btn)
    end
end
