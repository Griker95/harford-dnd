...
- Vive en `Harford/HarfordNamePlates.lua` y se carga despues de `HarfordUnitFrames.lua` en `Harford.toc`, porque reutiliza `HarfordUnitFrames.BuildResourceList`, `HarfordUnitFrames.ResourceColor` y `HarfordUnitFrames.C.TEX_STATUS`.
- Control de config: `HarfordConfig.Get("nameplates")`. `"on"` activa overlays; `"off"` oculta y limpia `npState`.
- Objetivo: pintar recursos Harford en nameplates sin tocar destructivamente el nameplate nativo.
- Soporta dos caminos:
  - **KuiNameplates activo**: usa `nameplate.kui`, `kui.HealthBar`, `kui.NameText`, `kui.IN_NAMEONLY` y `kui.unit`.
  - **Nativo WoW**: usa `nameplate.UnitFrame.healthBar` / `HealthBar` o campos equivalentes.
- En modo Kui normal, el overlay se parenta al `kuiFrame`, se ancla a `kui.HealthBar`, tapa la barra nativa con fondo opaco y muestra salud + primer recurso adicional. No reparentar a `HealthBar`, porque Kui puede ocultarla en modo name-only.
- En modo Kui name-only, el overlay se ancla a `kui.NameText`, queda a frame level bajo para que el texto del nombre siga encima, no muestra recurso secundario y colorea el nombre horizontalmente segun el porcentaje de HP. El fill usa el color actual de `NameText`.
- En nativo WoW sin Kui, crear overlay simple hijo de la `healthBar` nativa y pintar salud + primer recurso.
- Vida temporal: si `resources["Res_temp_health_Cur"] > 0`, se inyecta `hpData.tempCur` antes del render. El overlay `ov.tempBar` es un `StatusBar` usado como contenedor de absorcion. En modo Kui, `ov.tempBar._harfordUseKui = true` y la textura es `Interface\\AddOns\\Kui_Media\\t\\stippled-bar` (con `SetHorizTile(true)`). En modo nativo WoW, usa `Interface\\RaidFrame\\Shield-Fill` (con `SetHorizTile(false)`; esta textura se estira, no tilea — NO usar `Shield-Overlay` que causa huecos visibles). En ambos casos se llama `ApplyAbsorbTexture(ov.tempBar, ov.hpBar, curHp, maxHp, tempCur, 0.85)`. En name-only se oculta. **Patrones criticos de `ApplyAbsorbTexture` en nameplates**: (1) Pattern/glow se ocultan siempre (`_harfordAbsorbPattern:Hide()`, `_harfordAbsorbGlow:Hide()`): son frames cross-sibling que introducen anchors erroneos en el arbol de frames de la nameplate; (2) Spark: ancla `CENTER, frame, LEFT, realW * pct + 2` donde `realW = frame:GetWidth()` (o `ownerWidth` fallback si es 0) — **NO usar `fillTex:RIGHT`**: en Epsilon el fill texture de StatusBar tiene anchura completa basada en texcoords, por lo que `fillTex:RIGHT` apunta siempre al extremo derecho sin importar el valor de la barra; (3) **`OnSizeChanged` en `healthBar`**: Kui y los nameplates nativos WoW redimensionan la healthBar al seleccionar/deseleccionar; sin re-llamar `ApplyAbsorbTexture`, el spark queda en la posicion del tamaño anterior. Hookear `healthBar:HookScript("OnSizeChanged", ...)` y re-llamar `API.ApplyUnit(unit)`. Para nameplates nativos, usar guard `npState[unit]` en la closure: los nameplates se reciclan y la closure puede capturar un token de unidad antigua; si `npState[unit]` es nil (la unidad fue ocultada por `HideUnit`), no re-aplicar. No usar `TEX_STATUS` ni `StatusBar` plano para vida temporal en nameplates: visualmente parece un recurso normal y no un escudo de absorcion.
...
- Eventos: `NAME_PLATE_UNIT_ADDED` aplica overlay; `NAME_PLATE_UNIT_REMOVED` oculta y limpia la entrada. `HarfordUnitFrames` llama `HarfordNamePlates.RefreshAll()` cuando llegan recursos por `CHAT_MSG_ADDON`.
- Plugin Kui: si `KuiNameplates.NewPlugin` existe, registrar plugin `HarfordDnD`; mensajes `Show`/`Hide` aplican u ocultan overlay segun el frame reciclado por Kui.
- Debug relacionado:
