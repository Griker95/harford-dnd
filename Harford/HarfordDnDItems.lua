...
local function CacheResolvedItem(itemLink, resolved)
    ...
    if resolvedCache[itemLink] == nil then
        resolvedCacheOrder[#resolvedCacheOrder + 1] = itemLink
    end
    resolvedCache[itemLink] = resolved
    while CountTable(resolvedCache) > RESOLVED_CACHE_MAX and #resolvedCacheOrder > 0 do
...
local function StripAccentsMarkup(value)
    ...
    value = value:gsub("\195\128", "A"):gsub("\195\160", "a")
    value = value:gsub("\195\132", "A"):gsub("\195\164", "a")
    value = value:gsub("\195\137", "E"):gsub("\195\169", "e")
    value = value:gsub("\195\136", "E"):gsub("\195\168", "e")
    value = value:gsub("\195\139", "E"):gsub("\195\171", "e")
    value = value:gsub("\195\141", "I"):gsub("\195\173", "i")
    value = value:gsub("\195\140", "I"):gsub("\195\172", "i")
    value = value:gsub("\195\143", "I"):gsub("\195\175", "i")
    value = value:gsub("\195\147", "O"):gsub("\195\179", "o")
    value = value:gsub("\195\146", "O"):gsub("\195\178", "o")
    value = value:gsub("\195\150", "O"):gsub("\195\182", "o")
    value = value:gsub("\195\154", "U"):gsub("\195\186", "u")
    value = value:gsub("\195\153", "U"):gsub("\195\185", "u")
...
function API.RefreshPending()
        ...
        for _, entry in pairs(profile) do
            if type(entry) == "table" and entry.itemLink then
                ...
                if not cached or cached.pending then
                    ...
                    if resolved and not resolved.pending then changed = true end
                end
            end
        end
    end
    ...
    if type(profiles) == "table" then
        for _, p in pairs(profiles) do
            if type(p) == "table" then scan(p._equipment) end
        end
    end
    for _, profile in pairs(inspectData) do scan(profile) end
...
function API.GetPerfItems(reset)
    local snapshot = {
        ...
        cache = CountTable(resolvedCache),
...
if CreateFrame then
    ...
    eventFrame:SetScript("OnEvent", function()
        if API.RefreshPending() then
            ...
            if HarfordNamePlates and HarfordNamePlates.Refresh then HarfordNamePlates.Refresh() end
        end
    end)
end
