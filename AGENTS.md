...
Harford/
  ...
  HarfordDnD.lua              -- UI ficha, tiradas, recursos (consume los modulos DnD* anteriores)
...
- `HarfordDnD.lua` rozaba el limite de 200 locales de Lua 5.1. Se extrajeron datos, calculo puro y red a modulos `HarfordDnD*` para bajar de 194 a ~154 locales de file-scope. Verificar con `grep -c "^local " Harford/HarfordDnD.lua`.
- Contrato `HarfordDnDContext` (`Harford/HarfordDnDContext.lua`): es la "bisagra" que desacopla los demas modulos del chunk principal.
  - `HarfordDnDContext.State`: tabla unica y estable = el antiguo `SheetContext` (contexto temporal de ficha NPC: active, overrides, rollName, rollColor, actions, kind, spellProficiencyBonus, onAttackAnimation, onDamageRolled, ...). HarfordDnD.lua hace `local SheetContext = HarfordDnDContext.State` (misma referencia).
  - `HarfordDnDContext.Get(k, default)` / `Set(k, v)`: accesores ARC (los antiguos `ARCGET`/`ARCSET`). `Get` prioriza `State.overrides` y cae a `HarfordDnDStore`. HarfordDnD.lua hace `local ARCGET = HarfordDnDContext.Get` / `local ARCSET = HarfordDnDContext.Set` (sin tocar sus call-sites).
  - `HarfordDnDContext.SetSyncHook(fn)`: HarfordDnD.lua registra su `SyncRuntimeProfileRef`; Get/Set lo invocan para refrescar la referencia al runtime profile.
...
- **Flujo de daño con animación** (botón Daño en ficha NPC/DM):
  ...
  - **Victima jugador** (target jugador con animaciones activas):
    ...
    3. Aplicar daño: consumir primero `temp_health` via `RADJ`; el resto descontar de `health` via `RADJ`.
...
- **Nombre y color TRP3 en tiradas (`HarfordDnD`)**: las tiradas emitidas por el jugador local deben mostrar el nombre y color TRP3, no el nombre WoW ni el dorado fijo.
  - `GetRollDisplayName()`: si `SheetContext.rollName` está activo (contexto NPC), lo usa; si no, intenta `HarfordTRP3.GetUnitRPName("player")`; cae a `UnitName("player")`.
  - `SerializeRoll` incluye un **10.º campo** `nameColor` (hex 6 dígitos o string vacío).
  - `BroadcastRoll` establece `rollData.nameColor` antes de serializar: en contexto NPC usa `SheetContext.rollColor`; en modo normal usa `HarfordTRP3.GetUnitNameColor("player")`.
  - `DeserializeRoll` extrae `nameColor` de `parts[10]` y lo pasa en la tabla devuelta.
  - Los receptores que renderizan en chat aplican `nameColor` al nombre del emisor si está presente.
- **Sonido de tiradas TRP3 (`HarfordDnD`)**: en la copia local inspeccionada, `totalRP3/core/impl/slash.lua` llama a `TRP3_API.ui.misc.playSoundKit(36629, "SFX")` al terminar `TRP3_API.slash.rollDices(...)`; `totalRP3_Extended` reutiliza esa ruta. `BroadcastRoll` reproduce el mismo sound kit solo para tiradas locales reales (`type ~= "info"` y dados validos), usando la API TRP3 cuando existe para respetar su opcion de sonidos y `PlaySound(36629, "SFX")` como fallback. No moverlo a `DisplayRollInChat`: ese punto tambien renderiza resultados recibidos y haria sonar o duplicaria tiradas remotas.
...
- Reglas de seguridad:
  ...
  - Player health desde el menu admin va por su helper `AdjustResourceForName(...)` / `RADJ`, no por comando servidor. `HarfordDnDAPI.AdjustResourceForName` queda solo como puente pendiente para controles de `HarfordTurns`.
