...
| `Harford/HarfordCharacterSpellbook.lua` | Pestaña Conjuros (replica del libro de hechizos poblada por el compendio): `CreateSpellsPage`/`RefreshSpells`, filtros laterales + dropdown, botones Grimorio/Master. Extraida de HarfordCharacterPanel; recibe estado (`S`) + constantes del libro nativo via `Init(deps)`. Config de texturas propia `SPELLS_SKIN` | mediano |
