...
HarfordDnDCombat.PendingAttackReactions = HarfordDnDCombat.PendingAttackReactions or {}
HarfordDnDCombat.PreparedAttackReactions = HarfordDnDCombat.PreparedAttackReactions or {}
local nextAttackReactionId = HarfordDnDCombat.nextAttackReactionId or 0

local function SamePlayerName(left, right)
    local a = tostring(left or "")
    local b = tostring(right or "")
    if a == "" or b == "" then return false end
    if a == b then return true end
    local shortA = Ambiguate and Ambiguate(a, "short") or a:match("^[^%-]+") or a
    local shortB = Ambiguate and Ambiguate(b, "short") or b:match("^[^%-]+") or b
    return shortA == shortB
end

function HarfordDnDCombat.SetPreparedAttackReaction(protectedGuid, ownerName)
    -- PREPATTACK queda solo como compatibilidad de lectura de versiones
    -- antiguas. No existe una ventana de reaccion fiable entre impacto y dano
    -- entre clientes, asi que nunca volvemos a armar una repeticion remota.
    return false
end

-- Consulta al jugador defensor antes de confirmar un impacto. El resultado no
-- contiene dano ni datos de ficha: solo indica si consumio una reaccion propia.
function HarfordDnDCombat.RequestAttackReaction(defenderUnit, trigger, callback)
    -- Las reacciones automatizadas se limitan al dano local ya conocido
    -- (TriggerPreparedReaction). Barrera y equivalentes se anuncian y se
    -- resuelven en mesa; no hacemos round-trip de red para rerollear un d20.
    return false
end

function HarfordDnDCombat.HandleAttackReactionResult(requestId, used, sender)
    local pending = HarfordDnDCombat.PendingAttackReactions[tostring(requestId or "")]
    if not pending or not SamePlayerName(pending.target, sender) then return false end
    pending.finish(used == true)
    return true
end
...
function HarfordDnDCombat.ComputeSelfArmorClass()
    local formCA = HarfordDnDForms and HarfordDnDForms.GetArmorClass
        and HarfordDnDForms.GetArmorClass()
    if formCA then return math.floor(formCA) end
