...
API.ORDER = {
    ...
    "prone", "restrained", "stunned", "sleeping", "silenced", "rooted",
...
function API.ApplyToUnit(unit, conditionId, options)
    ...
    if def.auraId then
        if not (API.AdminHooks and API.AdminHooks.applyAura) then
            return false, "HarfordAdmin no esta disponible para aplicar la aura al NPC"
        end
        local ok, err = API.AdminHooks.applyAura({
            unit = unit,
    ...
    if HarfordDnDRolls and HarfordDnDRolls.Broadcast then
        ...
        HarfordDnDRolls.Broadcast({
            ...
            label = string.format("Salv %s %d (%d%+d vs CD %d) %s%s", ability, total, die, bonus, dc,
                saved and "EXITO: termina " or "FALLO: mantiene ", def and def.label or id),
        })
    end
    return saved
end
...
function API.HandleMessage(message, sender)
    ...
    if state then CacheRemoteState(state, sender); return true end
    return false
end
