...
-- Consume items del propio inventario (materiales de profesion). Envia `additem <id> -<qty>`
-- (cantidad negativa), verificado en Epsilon. El llamante debe comprobar antes que hay suficientes
-- (GetItemCount): el servidor no falla de forma observable si faltan, asi que la verificacion es
-- responsabilidad del core de profesiones.
function API.RemoveItem(itemId, quantity, opts)
    local safeItemId, itemErr = ToPositiveInteger(itemId, "itemId")
    if not safeItemId then
        return false, itemErr
    end
    ...
    return BuildAndSend("REMOVE_ITEM", { id = safeItemId, qty = safeQuantity }, opts)
end
