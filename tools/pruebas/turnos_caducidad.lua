...
-- Lo que hace que esto sea un COMBATE son dos cosas: combatientes, o un estado declarado. La
-- economia de turno y el movimiento son CONTABILIDAD -- se escriben en cada cambio de turno aunque
-- esten a cero, asi que casi siempre hay una tabla puesta. Tomarlas por combate hacia salir "se
-- retiro un combate abandonado" una y otra vez sin haber ninguno.
print("La contabilidad de turno no es un combate")
env.HarfordTurnOrderStore = { entries = {}, economia = { spent = { action = 1 }, ataques = 2 } }
chk("economia gastada sola: no anuncia nada", Purge(), false)
chk("  y se limpia en silencio", tostring(env.HarfordTurnOrderStore.economia), "nil")

env.HarfordTurnOrderStore = { entries = {}, movimiento = { metros = 12, corriendo = true } }
chk("movimiento solo: tampoco anuncia", Purge(), false)
chk("  y tambien se limpia", tostring(env.HarfordTurnOrderStore.movimiento), "nil")

-- Y lo que SI es un combate sigue caducando.
env.HarfordTurnOrderStore = { entries = {}, estado = "activo", economia = { spent = {} } }
