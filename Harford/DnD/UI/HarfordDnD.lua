...
-- (Extraidos a HarfordDnDRest en la fase C; Build devuelve las cuatro funciones.)
do
    local descansos = HarfordDnDRest and HarfordDnDRest.Build and HarfordDnDRest.Build({
        K = K, Print = Print, fmtSigned = fmtSigned, SheetContext = SheetContext,
        ResourceFrame = ResourceFrame,
        GetResourceCurrent = GetResourceCurrent,
        GetResourceMax = GetResourceMax,
        SetResourceCurrent = SetResourceCurrent,
        AdjustResourceCurrent = AdjustResourceCurrent,
        RefreshResourceFrame = function(...) if RefreshResourceFrame then return RefreshResourceFrame(...) end end,
        RefreshTargetResourceFrame = function(...) if RefreshTargetResourceFrame then return RefreshTargetResourceFrame(...) end end,
    }) or {}
    ApplyShortRest = descansos.ApplyShortRest
    ApplyLongRest = descansos.ApplyLongRest
    RollHitDieHeal = descansos.RollHitDieHeal
    RefreshRestMenu = descansos.RefreshRestMenu
end
...
-- (Extraido a HarfordDnDActionPanel en la fase C; Build devuelve el refresco.)
RefreshSheetActionPanel = HarfordDnDActionPanel and HarfordDnDActionPanel.Build and HarfordDnDActionPanel.Build({
    K = K, Print = Print, DoRoll = DoRoll, ConsumeMode = ConsumeMode,
    fmtSigned = fmtSigned, toN = toN, ColoredUnitName = ColoredUnitName,
    MakeArmorClassEditBox = MakeArmorClassEditBox,
    RefreshArmorClassBoxes = RefreshArmorClassBoxes,
...
if HarfordDnDBookActions and HarfordDnDBookActions.Init then
    HarfordDnDBookActions.Init({
        Print = Print,
        GetResourceCurrent = GetResourceCurrent,
        AdjustResourceCurrent = AdjustResourceCurrent,
        GetSpellAbilityKey = GetSpellAbilityKey,
        GetWeaponDef = GetWeaponDef,
        RefreshMainUI = RefreshMainUI,
        fmtSigned = fmtSigned,
        AnnounceFeatureById = AnnounceFeatureById,
        SheetContext = SheetContext,
    })
end
