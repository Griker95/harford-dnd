...
API.CLASSES[#API.CLASSES + 1] =
{
    ...
    subclasses = {
        { id = "disciplina", name = "Disciplina", desc = "Escudos y prevencion que sanan mitigando el daño.", features = {
                { id = "sac_disciplina_conjuros_1", level = 1, name = "Conjuros del llamado (Disciplina)", type = "informativo", grantedSpells = { "curar_heridas", "infligir_heridas" }, description = "Nivel 1: Curar heridas e Infligir heridas. Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
                { id = "sac_disciplina_conjuros_3", level = 3, name = "Conjuros del llamado (Disciplina)", type = "informativo", grantedSpells = { "oscuridad", "silencio" }, description = "Nivel 3: Oscuridad y Silencio. Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
                { id = "sac_disciplina_conjuros_5", level = 5, name = "Conjuros del llamado (Disciplina)", type = "informativo", grantedSpells = { "estrella_divina", "toque_vampirico" }, description = "Nivel 5: Estrella divina y Toque vampirico. Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
        ...
        { id = "sagrado", name = "Sagrado", desc = "Sanacion pura y restauradora con la Luz.", features = {
                { id = "sac_sagrado_conjuros_1", level = 1, name = "Conjuros del llamado (Sagrado)", type = "informativo", grantedSpells = { "bendicion", "curar_heridas" }, description = "Nivel 1: Bendicion (bendecir) y Curar heridas. Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
                { id = "sac_sagrado_conjuros_3", level = 3, name = "Conjuros del llamado (Sagrado)", type = "informativo", grantedSpells = { "restablecimiento_menor", "fuerza_brillante" }, description = "Nivel 3: Restablecimiento menor y Fuerza radiante (fuerza brillante). Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
                { id = "sac_sagrado_conjuros_5", level = 5, name = "Conjuros del llamado (Sagrado)", type = "informativo", grantedSpells = { "senal_de_esperanza", "revivir" }, description = "Nivel 5: Senal de esperanza (faro de esperanza) y Revivir. Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
            ...
            { id = "sac_sag_himno", level = 1, name = "Himno divino", type = "accion", uses = { max = 1, recharge = "short" }, description = "Acción: curación (= nivel x5 PG) repartida entre criaturas a 30 pies (no por encima de la mitad de su maximo). 1 uso por descanso.", uses = { max = 1, recharge = "short" }, effects = {} },
        ...
        { id = "sombra", name = "Sombra", desc = "Magia de la mente y energía sombria para destruir.", features = {
                { id = "sac_sombra_conjuros_1", level = 1, name = "Conjuros del llamado (Sombra)", type = "informativo", grantedSpells = { "brazos_de_hadar", "vacio_oscuro" }, description = "Nivel 1: Brazos de Hadar y Vacio oscuro. Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
                { id = "sac_sombra_conjuros_3", level = 3, name = "Conjuros del llamado (Sombra)", type = "informativo", grantedSpells = { "fuerza_fantasmal" }, description = "Nivel 3: Fuerza fantasmal (Descarga mental no esta en el compendio). Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
                { id = "sac_sombra_conjuros_5", level = 5, name = "Conjuros del llamado (Sombra)", type = "informativo", grantedSpells = { "toque_vampirico" }, description = "Nivel 5: Toque vampirico (Miedo no esta en el compendio). Siempre los tienes preparados y NO cuentan contra los conjuros que puedes preparar cada dia.", effects = {} },
            ...
            { id = "sac_som_legado", level = 1, name = "Legado del Vacio", type = "accion", actionKind = "voidLegacy", description = "Al dañar con un truco, daño psíquico extra = Mod. Carisma (con salvación de Sabiduría para seguir usandolo).", effects = {} },
            { id = "sac_som_forma", level = 6, name = "Forma de Sombra", type = "accion", uses = { max = 1, recharge = "short" }, description = "Acción adicional 1 minuto: si vas sin armadura sumas Mod. Carisma a la CA; daño necrótico a quien te golpee; tus conjuros ignoran resistencia necrótica/psiquica. 1 uso por descanso.", uses = { max = 1, recharge = "short" }, effects = {} },
    ...
    features = {
        ...
        { id = "sac_crear_ranura", level = 2, name = "Crear espacio de conjuro", type = "accion", description = "Accion adicional (Devocion): transformas puntos de fe en una ranura de conjuro. Coste: 2 puntos para una de 1.o, 3 para 2.o, 5 para 3.o, 6 para 4.o y 7 para 5.o. No puedes crear ranuras por encima del 5.o nivel.", actionKind = "slotConversion", slotConversion = { mode = "create", resource = "light_point" }, effects = {} },
        { id = "sac_convertir_ranura", level = 2, name = "Convertir espacio en puntos de fe", type = "accion", description = "Accion adicional (Devocion): gastas una ranura de conjuro y ganas puntos de fe iguales al nivel de la ranura.", actionKind = "slotConversion", slotConversion = { mode = "convert", resource = "light_point" }, effects = {} },
        ...
        { id = "sac_encadenar_no_muertos", level = 3, name = "Encadenar no muertos", type = "accion", category = "absolution", resourceKey = "light_point", resourceCost = 3, spendResourceOnAnnounce = true, description = "Absolucion. Accion: gastas 3 puntos de fe y presentas tu simbolo sagrado. Cada no-muerto que pueda verte u oirte en un radio de 9,1 metros debe hacer una salvacion de Sabiduria contra la CD de tus conjuros de sacerdote. Si falla, queda aturdido 1 minuto o hasta que reciba cualquier dano.", area = { shape = "sphere", sizeText = "9 m de radio", resolution = "save", saveAbility = "Sabiduria", success = "none", conditionId = "stunned", conditionDuration = "rounds", conditionTurns = 10, note = "Solo afecta a no-muertos que puedan verte u oirte. Termina antes si reciben cualquier dano." }, effects = {} },
