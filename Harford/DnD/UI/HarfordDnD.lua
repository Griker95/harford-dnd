...
local function DoRoll(label, baseBonus, profBonus, rollType, rollContext)
    local result = DoRollEx(label, baseBonus, profBonus, rollType, rollContext)
...
local function RollWeaponDamage(def, abilKey, maximizeDice, suppressAbilityDamage)
    ...
    if HarfordDnDFeatureEffects and HarfordDnDFeatureEffects.GetWeaponExtraDamage then
...
do
    ...
    SEC_TOP:RegisterEvent("MODIFIER_STATE_CHANGED")
    SEC_TOP:SetScript("OnEvent", function(self, event, key)
...
HarfordDnDStore.ApplyFailedSpecialDamage = function(diceText, damageType, targetUnit)
    ...
    if targetUnit == "player" then
        ...
        HarfordDnDCombat.ApplyWeaponDamageToTarget(total, false)
    end
    HarfordDnDRolls.Broadcast({ type = "damage", targetUnit = targetUnit, label = "Daño Embestida",
        total = total, dice = tostring(diceText), modifiers = "", critical = "", mode = "" })
    return total
end
...
local function RollRequestedSaveForSelf(ability, dc, outcome, auraId, responseTarget,
    ...
    if extraDamageDice and extraDamageDice ~= "" and responseTarget and HarfordSync and HarfordSync.SendRequestedSaveResult then
        HarfordSync.SendRequestedSaveResult(ADDON_PREFIX, responseTarget, saved, sourceGuid)
    end
    local rollData = {
        type = "info",
...
DoWeaponAttack = function(options)
    ...
    if man and man.afterHitSave then
        ...
        attackLabel = (offhand and "Ataque Offhand " or "Ataque ") .. WeaponRollName(def) .. wmodLabel .. maneuverLabel
            .. (options.labelSuffix and (" " .. options.labelSuffix) or "") .. targetLabel
    else
        attackLabel = manPrefix .. (offhand and "Ataque Offhand " or "Ataque ") .. WeaponRollName(def) .. wmodLabel
    ...
    if armorClass then
        ...
        if hit then
            onImpactOnce = function()
                ...
                -- de RollWeaponDamage porque este consume los daños condicionales activos.
                ...
                local damageTotal = RollWeaponDamage(def, abil, isCritical, options.suppressAbilityDamage == true)
...
do
    ...
    _G.DND5E_ARC_API.RollAbilityEx = function(abilityKey)
        ...
        local result = DoRollEx(a.short, HarfordDnDCalc.GetAbilityMod(a.key), 0, "ability", { actorUnit = "player", ability = a.key })
        result.kind = "ability"
    ...
    _G.DND5E_ARC_API.RollSaveEx = function(abilityKey)
        ...
        if not a then return nil end
        local base, prof = HarfordDnDCalc.GetSaveRollBonuses(a.key)
        local result = DoRollEx("Salv " .. a.short, base, prof, "save", { actorUnit = "player", ability = a.key })
        result.kind = "save"
        return SetLastRoll(result)
    end
    ...
    _G.DND5E_ARC_API.RollSkillEx = function(skillNameOrId)
        ...
        local base, prof = HarfordDnDCalc.GetSkillRollBonuses(s)
        local result = DoRollEx(s.name, base, prof, "ability", { actorUnit = "player", ability = s.ability, skill = s.id })
        result.kind = "skill"
        return SetLastRoll(result)
    end
    ...
    _G.DND5E_ARC_API.RollInitiativeEx = function()
        local result = DoRollEx("Iniciativa", HarfordDnDCalc.GetAbilityMod("Destreza"), HarfordDnDGetInitiativeMod(), "ability",
